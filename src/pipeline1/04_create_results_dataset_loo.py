...
def get_params_from_path(path): 
    path = path.split("/")[-1]
    usecase = int(path.split("_")[0])
    without = int(path.split("_")[-1].replace(".md", ""))
    return usecase, without

def get_metrics_for_answer(path): 
    path = str(path)
    questions = pd.read_csv(PATH_QUESTIONS)
    usecase, without = get_params_from_path(path)
...
if __name__ == "__main__":
    ...
    data.to_csv("ProvenanceCards/results/leaveoneout2.csv", sep=";")